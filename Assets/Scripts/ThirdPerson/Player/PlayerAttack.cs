...
public class PlayerAttack : MonoBehaviour
{
    ...
    private IEnumerator PerformAttack()
    {
        ...
        // Tenta acionar a animação
        if (anim != null)
        {
            StartCoroutine(SmoothLayerTransition(1.0f, 0.1f));
            anim.SetTrigger("attack"); // -> **verifique o nome exato do parâmetro no Animator**
            Debug.Log("[PlayerAttack] Trigger 'attack' set on Animator");
        }
        else
        {
            Debug.LogWarning("[PlayerAttack] Animator é null. A animação não será disparada.");
        }
        ...
        //yield return null;
        yield return StartCoroutine(SmoothLayerTransition(0f, 3f));
        isAttacking = false;
    }
